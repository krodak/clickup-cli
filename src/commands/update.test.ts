...
const mockUpdateTask = vi.fn().mockResolvedValue({ id: 't1', name: 'Task', status: { status: 'done', color: '' }, list: { id: 'l1', name: 'L1' }, assignees: [], url: '' })
...
describe('updateTask', () => {
  ...
  it('calls API with task id and description', async () => {
    ...
    const result = await updateTask({ apiToken: 'pk_t', teamId: 'team1' }, 't1', { description: 'new desc' })
  ...
  it('calls API with multiple fields at once', async () => {
    const { updateTask } = await import('./update.js')
    await updateTask({ apiToken: 'pk_t', teamId: 'team1' }, 't1', { name: 'New name', status: 'in progress' })
  ...
  it('throws when no fields provided', async () => {
    const { updateTask } = await import('./update.js')
    await expect(
      updateTask({ apiToken: 'pk_t', teamId: 'team1' }, 't1', {})
