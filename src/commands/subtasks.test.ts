...
describe('fetchSubtasks', () => {
  ...
  it('fetches parent task to get list ID', async () => {
    mockGetTask.mockResolvedValue({ id: 'p1', name: 'Parent', list: { id: 'l1', name: 'Roadmap' }, status: { status: 'open', color: '' }, assignees: [], url: '' })
    mockGetTasksFromList.mockResolvedValue([])
    const { fetchSubtasks } = await import('./subtasks.js')
  ...
  it('queries list with parent filter', async () => {
    ...
    const { fetchSubtasks } = await import('./subtasks.js')
    await fetchSubtasks({ apiToken: 'pk_t', teamId: 'team1' }, 'p1')
    expect(mockGetTasksFromList).toHaveBeenCalledWith('l1', expect.objectContaining({ parent: 'p1' }))
  })
