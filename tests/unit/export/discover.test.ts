...
describe('discoverTeamTasks', () => {
  ...
  it('resolves a space by id or case-insensitive name', async () => {
    ...
    const c = teamClient()
    expect(await resolveSpaceRef(c as never, 'ws1', 'sp1')).toEqual({ id: 'sp1', name: 'Atlas' })
    expect(await resolveSpaceRef(c as never, 'ws1', 'atlas')).toEqual({
      id: 'sp1',
      name: 'Atlas',
    })
    await expect(resolveSpaceRef(c as never, 'ws1', 'nope')).rejects.toThrow(
      /Space "nope" not found.*Atlas.*Other/,
    )
  ...
  it('walks folderless lists and folder lists, both active and archived, and records the hierarchy', async () => {
    ...
    const c = teamClient()
    const plan = await discoverTeamTasks(c as never, 'ws1', 'Atlas')

    expect(plan.slice).toEqual({ name: 'team-atlas', kind: 'team', scope: 'sp1' })
...
describe('discoverListTasks (roadmap / initiatives)', () => {
  function listClient() {
    return {
      getTeams: vi.fn().mockResolvedValue([{ id: 'ws1', name: 'krodak' }]),
      getListWithStatuses: vi.fn().mockResolvedValue({ id: 'l1', name: 'Atlas Product Roadmap' }),
  ...
  it('initiatives: only tasks with the initiative item id (subtasks follow via the engine)', async () => {
    ...
    expect(plan.slice.name).toBe('initiatives-atlas-product-roadmap')
