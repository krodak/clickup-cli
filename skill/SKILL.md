...
- All task IDs are stable alphanumeric strings (e.g. `86b8hzxdk`)
...
- `cu sprint` auto-detects active sprint from spaces where user has tasks, using view API and date range parsing from sprint names like "Kayenta Sprint 4 (2/12 - 2/25)"
- `--space` on sprint/tasks accepts partial name match (e.g. `--space Kay`)
...
cu task 86b8hzxdk --raw

# List subtasks of an initiative
cu subtasks 86b8hzxdk --json

# Update task status
cu update 86b8hzxdk -s "done"

# Create subtask under initiative (no --list needed)
cu create -n "Fix the thing" -p 86b8hzxdk

# Post a comment
cu comment 86b8hzxdk -m "Completed in PR #42"
```
